...
def main():
    ...
    if not os.path.exists(project_cfg.DOCUMENTATION_DIR + "/source"):
        os.makedirs(project_cfg.DOCUMENTATION_DIR + "/source")
...
def _build_commands():
    ...
    commands["create_initial_docs_command"] = (
        "sphinx-quickstart -p {PROJECT} -a {AUTHOR} -v {VERSION} --sep --makefile --batchfile -q "
        "--ext-autodoc --ext-doctest --ext-coverage --ext-imgmath --ext-todo "
        "--ext-intersphinx --ext-viewcode "
        "--extensions sphinxarg.ext {DOCS_DIR}".format(PROJECT=project_cfg.PROJECT_NAME,
                                                           AUTHOR=getpass.getuser(),
                                                           VERSION=1.0,
                                                           DOCS_DIR=project_cfg.DOCUMENTATION_DIR))

    commands["retrieve_source_api_command"] = (
        "sphinx-apidoc -o {DOCS_DIR}/source {SOURCE_DIR}".format(DOCS_DIR=project_cfg.DOCUMENTATION_DIR,
                                                                  SOURCE_DIR=project_cfg.SOURCE_CODE_DIR)
    )

    commands["build_docs_command"] = (
        "sphinx-build -b html {DOCS_DIR}/source {DOCS_DIR}/build".format(DOCS_DIR=project_cfg.DOCUMENTATION_DIR)
    )
...
def _manually_append_source_path_to_sphinx_config():
    ...
    """
    with open(project_cfg.DOCUMENTATION_DIR + "/source/conf.py", "a+", encoding="UTF-8") as sphinx_config:
        sphinx_config_abs_location = os.path.realpath(os.path.dirname(sphinx_config.name))
        source_relative_to_sphinx_config = os.path.relpath(project_cfg.SOURCE_CODE_DIR, sphinx_config_abs_location)
